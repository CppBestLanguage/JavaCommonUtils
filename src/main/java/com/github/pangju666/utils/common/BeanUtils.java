...
public class BeanUtils {
    ...
    public static <T, R> List<R> convertList(List<T> entityList, Function<T, R> convertFunc) {
        return entityList.stream().map(convertFunc).collect(Collectors.toList());
    }

    private static <T, R> BeanCopier getBeanCopier(TypeToken<T> sourceTypeToken, TypeToken<R> targetTypeToken) {
        // 获取源对象类型
    ...
    private static <T> Class<T> getGenericClass(TypeToken<T> value) {
